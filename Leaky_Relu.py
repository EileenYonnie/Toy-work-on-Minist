# -*- coding: utf-8 -*-
"""
Created on Thu Sep 27 20:44:00 2018
...
class Data:
    def __init__(self):
        ...
        self.read_train_images( '/Users/apple/Desktop/minist/train-images.idx3-ubyte')
        self.read_train_labels( '/Users/apple/Desktop/minist/train-labels.idx1-ubyte')

        self.train_data = np.append( self.train_img_list, self.train_label_list, axis = 1 )


        self.read_test_images('/Users/apple/Desktop/minist/t10k-images.idx3-ubyte')
        self.read_test_labels('/Users/apple/Desktop/minist/t10k-labels.idx1-ubyte')
    ...
    def predict(self):
        ...
        hidden_layer3 = np.maximum(0, np.matmul(hidden_layer2, self.W3) + self.b3)


        scores = np.maximum(0, np.matmul(hidden_layer3, self.W4) + self.b4)
    ...
    def train_network(self, img_batch_list, label_batch_list):
        ...
        loss = np.mean( loss_list, axis=0 )[0] + \
               0.1 * self.reg_factor * np.sum( self.W1 * self.W1 ) + \
               0.1 * self.reg_factor * np.sum( self.W2 * self.W2 ) + \
               0.1 * self.reg_factor * np.sum( self.W3 * self.W3 ) + \
               0.1 * self.reg_factor * np.sum( self.W4 * self.W4 )
    ...
    def init_network(self):
        self.W1 = 0.01 * np.random.randn( 28 * 28, 256 )
        self.b1 = 0.01 * np.random.randn( 1, 256 )

        self.W2 = 0.01 * np.random.randn( 256, 128 )
        self.b2 = 0.01 * np.random.randn( 1,   128 )

        self.W3 = 0.01 * np.random.randn( 128, 64 )
        self.b3 = 0.01 * np.random.randn( 1,  64 )

        self.W4 = 0.01 * np.random.randn(64, self.K)
        self.b4 = 0.01 * np.random.randn(1,  self.K)
